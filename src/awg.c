...
uint32_t
awg_range_pick (const AwgRange *r, uint32_t fallback)
{
  return r->lo || r->hi ? range_pick (r) : fallback;
}
...
awg_set (Awg *a, const char *key, const char *value)
{
  ...
  if (!strcmp (key, "rekey_after_time"))
    return awg_range_set (&a->rekey_after, value);
  if (!strcmp (key, "rekey_timeout"))
    return awg_range_set (&a->rekey_timeout, value);
  if (!strcmp (key, "reject_after_time"))
    return awg_range_set (&a->reject_after, value);
  if (!strcmp (key, "keepalive_timeout"))
    return awg_range_set (&a->keepalive_timeout, value);
  if (!strcmp (key, "max_handshake_attempts"))
  ...
  if (a->content_pad.lo || a->content_pad.hi)
    {
      ...
      return mtu && pad > mtu - unit ? mtu - unit : pad;
    }
  pad = (unit + 15U) & ~15U;
  if (mtu && pad > mtu)
    pad = mtu;
...
static int
tag_get (const char *tag, char name[4], char *value, size_t cap)
{
  const char *p = tag;
  size_t n = 0;
  ...
  while (*p)
    {
      ...
      if (!end || !(n = (size_t)(end - p)))
        return -EINVAL;
      char *arg = malloc (n + 1U);
      char *value = malloc (n + 1U);
      if (!arg || !value)
        {
          free (value);
          free (arg);
      ...
      arg[n] = '\0';
      p = end + 1;
      if (tag_get (arg, tag, value, n + 1U) < 0)
        {
          free (value);
          free (arg);
          return -EINVAL;
      ...
      if (!strcmp (tag, "b"))
        {
          ...
          if (!chars || (chars & 1U) || used + chars / 2U > cap
